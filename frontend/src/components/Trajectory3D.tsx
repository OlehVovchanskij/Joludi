...
export default function Trajectory3D({
  ...
  const plotDataWithUi = plotData.map((trace) => {
    const nextTrace = { ...trace };

    if (
      typeof trace.marker === "object" &&
      trace.marker &&
      !Array.isArray(trace.marker)
    ) {
      const marker = trace.marker as Record<string, unknown>;
      const baseColorbar =
        typeof marker.colorbar === "object" &&
        marker.colorbar &&
        !Array.isArray(marker.colorbar)
      ...
      <Plot
        ...
        layout={{
          ...
          legend: {
            ...
            bordercolor: "rgba(27, 35, 33, 0.16)",
            borderwidth: 1,
          },
          scene: {
            ...baseScene,
